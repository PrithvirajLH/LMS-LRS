...
const statusConfig: Record<CourseStatus, { label: string; bg: string; text: string; dot: string }> = {
  ...
  completed: { label: "COMPLETED", bg: "var(--slate-50)", text: "var(--slate-600)", dot: "var(--slate-400)" },
...
export function CourseCardHover({
  title, category, duration, credits, progress, status, dueIn, currentModule, color, index,
  isHovered, onHover, onLeave,
}: CourseCardHoverProps) {
      ...
      >
        ...
        <div className="p-6">
          ...
          {currentModule && status !== "completed" && (
            <p
              className="mt-2"
              style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "var(--teal-400)" }}
            >
              {currentModule}
            </p>
          )}
          ...
          {status !== "not_started" && (
            <div className="mt-4">
              <div className="flex items-center justify-between mb-1.5">
                <span style={{ fontFamily: "var(--font-label)", fontSize: "10px", letterSpacing: "var(--tracking-wider)", textTransform: "uppercase" as const, color: "var(--text-muted)" }}>
                  Progress
                </span>
          ...
          <div className="flex items-center justify-between mt-5 pt-4" style={{ borderTop: "1px solid var(--border-default)" }}>
            <div className="flex items-center gap-3">
              <span style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "var(--text-muted)" }}>{duration}</span>
              <span style={{ color: "var(--stone-200)" }}>·</span>
              <span style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "var(--text-muted)" }}>{credits} credits</span>
              {dueIn && (
                <>
                  <span style={{ color: "var(--stone-200)" }}>·</span>
                  <span style={{ fontFamily: "var(--font-body)", fontSize: "12px", fontWeight: status === "overdue" ? 700 : 400, color: status === "overdue" ? "var(--amber-600)" : "var(--text-muted)" }}>
                    {dueIn}
                  </span>
                </>
              )}
...
// Grid wrapper that manages hover state
export function CourseCardHoverGrid({ children }: { children: (hoveredIndex: number | null, setHovered: (i: number | null) => void) => React.ReactNode }) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  return <>{children(hoveredIndex, setHoveredIndex)}</>;
}
